...
// The `storage.local` key names below are immutable: renaming one strands the
// data and VAULT_CIPHER_KEY bricks vaults. See docs/architecture/storage-keys.md
...
export async function getExistingMainStoreSingletonOrInit() {
  try {
    // In parallel, never sequentially: both reads run on every
    // `runtime.onMessage`, to serve data used only when the singleton is null.
    const [
      {
    ...
    if (storeSingleton == null) {
      if (isMockStateEnable) {
        ...
        storeSingleton = createStore({
          ...
          // reducer uses, so a poisoned persisted flag can't survive a restart.
          ...
          trustedWasm,
          csprNameExpirations,
          // Present before any handler can run: on Chrome/Edge this rehydrates
          // `windowManagement.requests` across an MV3 restart.
          windowManagement: {
      ...
      // The pair, not the slice reference: the window-id case reducers return a
      // fresh slice even for a value-equal write.
      ...
      // Passed the hydrated snapshot rather than reading `getState()` itself: a
      // request registered after this line must survive its registration→attach gap.
