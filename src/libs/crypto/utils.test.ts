...
// The validators throw exactly when the argument is NOT of the expected type, so whatever they
// stringify is the unexpected value — potentially key material, on its way to a visible banner.
describe('crypto validators — error messages never carry the value', () => {
  ...
  // The dangerous shape: raw key bytes reaching a validator that expects hex. Rendering the
  // argument would Array-join them into a decimal dump — a full plaintext leak of the key.
  it('convertHexToBytes rejects key bytes without dumping them as a decimal list', () => {
    ...
    const decimalDump = String(KEY_BYTES);
