...
| ------------------------------ | ---------------- | ------------------------------------------------------------------------------------------------ | -------------------------------------------- | ---------------------------------------------------------------------- |
| `VAULT_CIPHER_KEY`             | `zazXu8w9GyCtxZ` | The encrypted vault blob (`vaultCipher` slice) — accounts, keys, and other secret vault contents | **Yes** — the only secret material persisted | No — AES-256-GCM ciphertext, `base64(iv[12]\|\|ciphertext\|\|tag[16])` |
| `KEYS_KEY`                     | `2yNVAEQJB5rxMg` | Password/salt hashes and key-derivation salt hash (`keys` slice)                                 | No (hashes, not the password or key itself)  | Yes                                                                    |
